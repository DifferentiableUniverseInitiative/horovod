...
void AllgatherOp::MemcpyInFusionBuffer(
  ...
  int64_t offset = displcmnts[global_state_->controller[global_state_->current_nccl_stream]->GetRank()] * element_size;
  for (auto& e : entries) {
    void* buffer_data_at_offset = (uint8_t*)buffer_data + offset;
    MemcpyEntryInFusionBuffer(entries, e, buffer_data_at_offset);
    offset += e.tensor->size();
...
Status JoinOp::Execute(std::vector<TensorTableEntry>& entries,
  ...
  if (global_state_->joined[global_state_->current_nccl_stream]) {
    global_state_->tensor_queue.RemoveJoinTensor();
    global_state_->joined[global_state_->current_nccl_stream] = false;
  }
  return Status::OK();
}
