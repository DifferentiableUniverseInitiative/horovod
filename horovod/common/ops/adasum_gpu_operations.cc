...
AdasumGpuAllreduceOp::AdasumGpuAllreduceOp(MPIContext* mpi_context,
    ...
    : AdasumMPI(mpi_context, global_state),
      NCCLAllreduce(nccl_context, gpu_context, global_state, iComm, Communicator::LOCAL) {
...
Status AdasumGpuAllreduceOp::Execute(std::vector<TensorTableEntry>& entries,
  ...
  return NcclHierarchical(entries, response, iComm);
}
...
AdasumGpuAllreduceOp::NcclHierarchical(std::vector<TensorTableEntry>& entries,
  ...
  nccl_device_map.reserve(
      global_state_->controller[iComm]->GetLocalCommRanks().size());
  for (size_t rank : global_state_->controller[iComm]->GetLocalCommRanks()) {
  ...
  if (global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous() || is_root_rank) {
    ...
    DispatchFusedAllreduce(
        ...
        global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous()
            ? MPI_COMM_WORLD
