...
Status MPIHierarchicalAllgather::Execute(std::vector<TensorTableEntry>& entries, const Response& response) {
  ...
  if (global_state_->shared_buffer == nullptr || global_state_->shared_buffer_size < total_size_in_bytes) {
    ...
    int64_t window_size = global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank() == 0 ? total_size_in_bytes : 0;
    MPI_Win_allocate_shared(window_size,
                            ...
                            &mpi_context_->window);
    if (global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank() != 0) {
      int disp_unit;
      MPI_Aint winsize;
  ...
  int cross_size = global_state_->controller[global_state_->current_nccl_stream]->GetCrossSize();
  ...
  if (global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous()) {
    for (int i = 0; i < global_state_->controller[global_state_->current_nccl_stream]->GetCrossSize(); ++i) {
      cross_recvcounts[i] = recvcounts[local_size * i + local_rank];
      cross_displcmnts[i] = displcmnts[local_size * i + local_rank];
    }
  } else if (global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank() == 0) {
    // In this case local rank 0 will allgather with all local data
    int offset = 0;
    for (int i = 0; i < cross_size; ++i) {
      for (int j = offset; j < offset + global_state_->controller[global_state_->current_nccl_stream]->GetLocalSizeAtCrossRank(i);
           ++j) {
        cross_recvcounts[i] += recvcounts[j];
      }
      cross_displcmnts[i] = displcmnts[offset];
      offset += global_state_->controller[global_state_->current_nccl_stream]->GetLocalSizeAtCrossRank(i);
    }
  }

  timeline.ActivityStartAll(entries, MEMCPY_IN_SHARED_BUFFER);

  int rank = global_state_->controller[global_state_->current_nccl_stream]->GetRank();
  for (size_t ec = 0; ec < entries.size(); ++ec) {
    auto& e = entries[ec];
  ...
  if (global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous() || global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank() == 0) {
