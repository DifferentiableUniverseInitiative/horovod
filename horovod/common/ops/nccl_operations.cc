...
void NCCLOpContext::InitNCCLComm(const std::vector<TensorTableEntry>& entries,
  ...
  LOG(DEBUG, "InitNCCLComm() start, iComm = " << iComm);
  ncclComm_t& nccl_comm = nccl_context_->nccl_comms[iComm][nccl_device_map];
  if (nccl_comm == nullptr) {
    ...
    PopulateNCCLCommStrategy(nccl_rank, nccl_size, nccl_id_bcast_comm, iComm);
    ...
    global_state_->controller[iComm]->Bcast((void*)&nccl_id, sizeof(nccl_id), 0,
                                         nccl_id_bcast_comm);
    ...
    global_state_->controller[iComm]->Barrier(Communicator::GLOBAL);

    timeline.ActivityEndAll(entries);
  }

  nccl_comm_ = &nccl_comm;
  LOG(DEBUG, "InitNCCLComm() end, iComm = " << iComm);
}
...
void NCCLOpContext::PopulateNCCLCommStrategy(int& nccl_rank, int& nccl_size,
  ...
  if (communicator_type_ == Communicator::GLOBAL) {
    nccl_rank = global_state_->controller[iComm]->GetRank();
    nccl_size = global_state_->controller[iComm]->GetSize();
  } else if (communicator_type_ == Communicator::LOCAL) {
    nccl_rank = global_state_->controller[iComm]->GetLocalRank();
    nccl_size = global_state_->controller[iComm]->GetLocalSize();
  } else {
...
Status NCCLAllreduce::Execute(std::vector<TensorTableEntry>& entries,
  ...
  nccl_op_context_.InitNCCLComm(entries, response.devices(), iComm);
...
NCCLHierarchicalAllreduce::Execute(std::vector<TensorTableEntry>& entries,
  ...
  nccl_device_map.reserve(
      global_state_->controller[global_state_->current_nccl_stream]->GetLocalCommRanks().size());
  for (int rank : global_state_->controller[global_state_->current_nccl_stream]->GetLocalCommRanks()) {
    nccl_device_map.push_back(response.devices()[rank]);
  }

  gpu_op_context_.InitGPU(entries);
  nccl_op_context_.InitNCCLComm(entries, nccl_device_map, iComm);
  gpu_op_context_.InitGPUQueue(entries, response);
  ...
  int local_size = global_state_->controller[global_state_->current_nccl_stream]->GetLocalSize();
  int local_rank = global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank();
  ...
  if (global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous() && entries.size() > 1) {
  ...
  int64_t num_elements_per_rank = global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous()
  ...
  int64_t num_elements_remaining = global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous()
                                       ? num_elements % local_size
                                       : num_elements;
  ...
  int root_rank =
      global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous() ? local_size - 1 : 0;
  ...
  if (global_state_->controller[global_state_->current_nccl_stream]->IsHomogeneous() || is_root_rank) {
...
Status NCCLBroadcast::Execute(std::vector<TensorTableEntry>& entries,
  ...
  void* data_ptr;
  if (global_state_->controller[global_state_->current_nccl_stream]->GetRank() == e.root_rank) {
...
Status NCCLAllgather::Execute(std::vector<TensorTableEntry>& entries,
  ...
  int global_size = global_state_->controller[global_state_->current_nccl_stream]->GetSize();
  int global_rank = global_state_->controller[global_state_->current_nccl_stream]->GetRank();
...
bool NCCLAlltoall::Enabled(const ParameterManager& param_manager,
                              const std::vector<TensorTableEntry>& entries,
                              const Response& response) const {
  LOG(TRACE, "NCCLAlltoall::Enabled()");
  return true;
}

Status NCCLAlltoall::Execute(std::vector<TensorTableEntry>& entries,
                             const Response& response) {
  LOG(TRACE, "NCCLAlltoall::Execute() OLD VERSION entered.");
  return Status::OK();
}
...
Status NCCLAlltoall::Execute(std::vector<TensorTableEntry>& entries,
                             const Response& response,
			     int iComm) {
  LOG(TRACE, "NCCLAlltoall::Execute() start.");
#ifdef NCCL_P2P_SUPPORTED
  LOG(TRACE, "NCCLAlltoall::Execute() NCCL_P2P_SUPPORTED.");
  assert(entries.size() == 1);
  auto e = entries[0];

  LOG(TRACE, "NCCLAlltoall::Execute() before InitGPU");
  gpu_op_context_.InitGPU(entries, iComm);
  LOG(TRACE, "NCCLAlltoall::Execute() after InitGPU");
  nccl_op_context_.InitNCCLComm(entries, response.devices(), iComm);
  gpu_op_context_.InitGPUQueue(entries, response, iComm);
  LOG(TRACE, "NCCLAlltoall::Execute() after InitGPUQueue.");
  ...
  LOG(TRACE, "NCCLAlltoall::Execute() after PrepareOutputAndParams.");

  auto world_size = global_state_->controller[iComm]->GetSize();
  ...
  for (int i = 0; i < world_size; ++i) {
    LOG(TRACE, "NCCLAlltoall::Execute() loop start");
    if (recvcounts[i] > 0) {
      LOG(TRACE, "NCCLAlltoall::Execute() ncclRecv() start.");
      auto nccl_result = ncclRecv((uint8_t*) e.output->data() + rdispls[i] * DataType_Size(e.tensor->dtype()),
                                  ...
                                  *nccl_op_context_.nccl_comm_, *gpu_op_context_.stream);
      LOG(TRACE, "NCCLAlltoall::Execute() ncclRecv() end.");
      nccl_context_->ErrorCheck("ncclRecv", nccl_result, *nccl_op_context_.nccl_comm_);
    }

    if (sendcounts[i] > 0) {
      LOG(TRACE, "NCCLAlltoall::Execute() ncclSend() start.");
      auto nccl_result = ncclSend((uint8_t*) e.tensor->data() + sdispls[i] * DataType_Size(e.tensor->dtype()),
                             ...
                             *nccl_op_context_.nccl_comm_, *gpu_op_context_.stream);
      LOG(TRACE, "NCCLAlltoall::Execute() ncclSend() ended.");
      nccl_context_->ErrorCheck("ncclSend", nccl_result, *nccl_op_context_.nccl_comm_);
    }
  }
  nccl_context_->ErrorCheck("ncclGroupEnd", ncclGroupEnd(), *nccl_op_context_.nccl_comm_);
  LOG(TRACE, "NCCLAlltoall::Execute() after nccl_context->ErrorCheck 2.");
...
void NCCLOpContext::InitNCCLComm(const std::vector<TensorTableEntry>& entries,
  ...
  if (nccl_comm == nullptr) {
    ...
    global_state_->controller[global_state_->current_nccl_stream]->Bcast((void*)&nccl_id, sizeof(nccl_id), 0,
                                         nccl_id_bcast_comm);
    ...
    // Barrier helps NCCL to synchronize after initialization and avoid
    // deadlock that we've been seeing without it.
    global_state_->controller[global_state_->current_nccl_stream]->Barrier(Communicator::GLOBAL);
...
void NCCLOpContext::PopulateNCCLCommStrategy(int& nccl_rank, int& nccl_size,
                                             Communicator& nccl_id_bcast_comm) {
  if (communicator_type_ == Communicator::GLOBAL) {
    nccl_rank = global_state_->controller[global_state_->current_nccl_stream]->GetRank();
    nccl_size = global_state_->controller-[global_state_->current_nccl_stream]>GetSize();
  } else if (communicator_type_ == Communicator::LOCAL) {
    nccl_rank = global_state_->controller[global_state_->current_nccl_stream]->GetLocalRank();
    nccl_size = global_state_->controller[global_state_->current_nccl_stream]->GetLocalSize();
  } else {
    throw std::logic_error("Communicator type " + std::to_string(communicator_type_) +
                            " is not supported in NCCL mode.");
  }
...
Status NCCLAlltoall::Execute(std::vector<TensorTableEntry>& entries,
  ...
  auto world_size = global_state_->controller[global_state_->current_nccl_stream]->GetSize();
  const void* sendbuf = e.tensor->data();
  void* buffer_data = (void*) e.output->data();
